...
export async function extractEntities(params) {
  ...
  try {
    ...
    - childGender: "male" OR "female" OR null
    - locationArea: string (city name)
    - budgetMin: number or null
    - budgetMax: number or null
    - budgetSingle: number or null
    ...
    - wellbeing_needs: array or null
    - childrenJson: string or null
    - curriculumPreference: array or null
    - programPreferences: array or null
    - religiousPreference: string or null
    - boardingPreference: string or null
    - genderPreference: "Co-Ed" OR "All Boys" OR "All Girls" OR null
    - classSize: string or null
    - requestedSchools: array of school names or null
    - financialAidInterest: boolean or null
    - specialNeeds: array or null
    - intentSignal: string (required - one of: 'continue', 'request-brief', 'request-results', 'edit-criteria', 'ask-about-school', 'back-to-results', 'restart', 'off-topic')
    ...
    Return ONLY valid JSON. Do NOT explain.`;
        ...
        console.error('[FALLBACK ERROR] InvokeLLM extraction also failed:', fallbackError.message);
        result = {}; // Ensure result is always an object
