...
ACTIONS (JSON output): Your response is structured as JSON with a 'message' field and an 'actions' array. When the user EXPLICITLY asks to add this school to their shortlist, save it, bookmark it, or keep it, include an ADD_TO_SHORTLIST action with schoolId set to '${selectedSchoolId}'. When they ask to see their shortlist, include an OPEN_PANEL action with panel='shortlist'. Only emit actions when user intent is clear and explicit - do NOT auto-add schools or assume intent. If no action is requested, return actions as an empty array []. Available action types: ADD_TO_SHORTLIST (requires schoolId), OPEN_PANEL (requires panel), EXPAND_SCHOOL (requires schoolId).`;
