import { SchoolClaim, EmailLog } from '@/lib/entities-server'
...
export async function sendClaimEmail(params: {
  ...
  const { emailType, schoolData, test_mode = false, test_scenario = null } = params;
  let claimData = params.claimData;

  let subject: string;
  let body: string;
  const APP_URL = process.env.APP_URL || 'https://app.nextschool.ca';
  ...
  if (emailType === 'VERIFICATION_CODE') {
    if (!schoolData.claimId) {
      throw Object.assign(new Error('claimId is required for VERIFICATION_CODE type'), { statusCode: 400 });
    }
  ...
  switch (emailType) {
    ...
    case 'CLAIM_EXPIRED':
      ...
      body = `${baseStyles}
        ...
        <div style="text-align: center; margin: 32px 0;">
          <a href="${APP_URL}/claim-school?schoolId=${schoolData.id}"
             style="display: inline-block; background: #0D9488; color: #ffffff; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 600;">
    ...
    default:
      throw Object.assign(new Error('Invalid email type'), { statusCode: 400 });
  }
  ...
  if (test_mode) {
    ...
    return { success: true, reason: 'test_blocked' };
  }
  ...
  try {
    ...
    return { success: true, ...(emailType === 'VERIFICATION_CODE' && { expiresInMinutes: 15 }) };
  } catch (emailErr: any) {
