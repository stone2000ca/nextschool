...
function mergeProfileData(existing: any, extracted: any): any {
...
async function extractPhaseData(message: string, currentPhase: string, _profile: any): Promise<any> {
...
function checkRequiredFields(phase: string, profile: any): boolean {
...
async function generatePhaseResponse(message: string, nextPhase: string, profile: any, _requiredFieldsMet: boolean): Promise<string> {
...
async function generateFamilyBrief(profile: any): Promise<string> {
...
export async function onboardUser(params: {
  ...
  const { message, userId, conversationHistory, familyProfileData } = params;
  ...
  if (!profile) {
    ...
    const created = await FamilyProfile.create(profile);
    profile = created;
  }
  ...
  if (currentPhase === 'confirm_brief' && requiredFieldsMet) {
    ...
    nextPhase = 'confirm_brief';

    // Generate The Brief
    try {
      aiMessage = await generateFamilyBrief(profile);
    ...
    aiMessage = await generatePhaseResponse(message, nextPhase, profile, requiredFieldsMet);
  }
