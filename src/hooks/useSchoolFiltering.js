...
/**
 * Hook for school filtering, sorting, and distance calculation.
 *
 ...
 * @returns {Object} { filteredSchools, sortField, sortDirection, setSortField, setSortDirection, showDistances, applyDistances }
 */
export function useSchoolFiltering(schools, conversationContext) {
  ...
  const getFilteredAndSortedSchools = useCallback(() => {
    try {
  ...
  }, [schools, conversationContext, sortField, sortDirection]);
  ...
  /**
   * Reset sort to relevance (e.g., when new schools arrive).
   */
  ...
  return {
    filteredSchools: getFilteredAndSortedSchools(),
    sortField,
    sortDirection,
    setSortField,
    setSortDirection,
