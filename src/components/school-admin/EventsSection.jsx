...
function FreeTierTeaser({ school }) {
  ...
  useEffect(() => {
    base44.entities.SchoolEvent.filter({ school_id: school.id, source: 'ai_enriched' })
      .then(setAiEvents)
      .catch(() => {});
  }, [school.id]);
...
function PremiumEventsManagement({ school }) {
  ...
  const loadEvents = async () => {
    setLoading(true);
    const data = await base44.entities.SchoolEvent.filter({ school_id: school.id });
  ...
  const handleSave = async (formData) => {
    if (editing) {
      await base44.entities.SchoolEvent.update(editing.id, { ...formData, source: 'school_portal', is_confirmed: true });
    } else {
      await base44.entities.SchoolEvent.create({ ...formData, school_id: school.id, source: 'school_portal', is_confirmed: true, is_active: true });
    }
    setEditing(null);
    await loadEvents();
  };
