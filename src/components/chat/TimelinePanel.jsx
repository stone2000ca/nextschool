...
export default function TimelinePanel({ shortlist, onClose }) {
  ...
  useEffect(() => {
    async function fetchEvents() {
      ...
      try {
        ...
        console.error('[TimelinePanel] Failed to fetch events:', err);
        setUpcomingEvents([]);
      } finally {
        setEventsLoaded(true);
      }
    ...
    <div className="h-full flex flex-col" style={{ background: '#1E1E30', borderLeft: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-l-4 border-l-teal-400" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-white">Timeline & Events</h2>
        </div>
      ...
      <ScrollArea className="flex-1">
        {shortlist.length === 0 ? (
          <div className="text-center py-12 px-4">
            ...
            <p className="text-sm text-slate-400">No schools shortlisted yet.</p>
            <p className="text-xs text-slate-500 mt-1">Add schools to your shortlist to see their timelines and events.</p>
          </div>
        ) : (
          <>
            {/* Application Timeline — 2+ schools only */}
            {shortlist.length >= 2 && (
