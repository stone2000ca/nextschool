...
// Hook: useMessageHandler
// Purpose: Encapsulate handleSendMessage logic using ChatContext
// Returns: handleSendMessage function
export function useMessageHandler() {
  const context = useChatContext();

  const handleSendMessage = useCallback(
    async (messageText, explicitSchoolId = null, displayText = null) => {
      ...
      try {
        ...
        // (This is handled in Consultant.jsx via state)
        ...
        if (!isViewingSchoolDetail && response.data.state) {
          ...
          setCurrentView(
            response.data.state === STATES.WELCOME ? 'chat' :
            response.data.state === STATES.DISCOVERY ? 'chat' :
            response.data.state === STATES.BRIEF ? 'chat' :
            response.data.state === STATES.RESULTS ? 'schools' :
            response.data.state === STATES.DEEP_DIVE ? 'detail' :
            'chat'
          );
        ...
        if (
          ...
          // (This is handled in Consultant.jsx via hasAutoPopulatedShortlist ref)
        }

        // Create ChatSession when brief is confirmed and transitioning to RESULTS
        if (response.data.state === STATES.RESULTS) {
          try {
            const matchedSchoolIds = response.data.schools
              ? response.data.schools.map(s => s.id)
              : [];
        ...
        if (isAuthenticated && user) {
          const { extractAndSaveMemories } = await import(
            '@/components/utils/memoryManager'
          );
        ...
        if (isAuthenticated && currentConversation && currentConversation.id) {
          ...
          if (
            ...
            try {
              const titleResult = await base44.functions.invoke(
          ...
          if (
            finalMessages.filter(m => m.role === 'user').length % 5 === 0
          ) {
            try {
